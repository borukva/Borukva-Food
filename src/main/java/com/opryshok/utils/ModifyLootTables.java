        ...
        LootTableEvents.MODIFY.register((key, tableBuilder, source, registries) -> {
            ...
            if ((key.getValue() == LootTables.SNIFFER_DIGGING_GAMEPLAY.getValue())) {
                ...
                if (!FarmersDelightCompat.IS_LOADED) {
                    pool.with(ItemEntry.builder(ModItems.ONION_SEEDS))
                        .with(ItemEntry.builder(ModItems.RICE_PANICLE));
                }
                tableBuilder.pool(pool);
            }
            if ((key.getValue() == LootTables.PILLAGER_OUTPOST_CHEST.getValue()) && !FarmersDelightCompat.IS_LOADED){
                LootPool.Builder pool = LootPool.builder()
                        .rolls(ConstantLootNumberProvider.create(1))
                        .conditionally(RandomChanceLootCondition.builder(1f))
            ...
            if ((key.getValue() == LootTables.SIMPLE_DUNGEON_CHEST.getValue()) && !FarmersDelightCompat.IS_LOADED){
                LootPool.Builder pool = LootPool.builder()
                        .rolls(ConstantLootNumberProvider.create(1))
            ...
            if ((key.getValue() == LootTables.TRAIL_RUINS_COMMON_ARCHAEOLOGY.getValue())){
                tableBuilder.modifyPools(builder -> {
                    ...
                    if (!FarmersDelightCompat.IS_LOADED) {
                        builder.with(ItemEntry.builder(ModItems.TOMATO_SEEDS).weight(2));
                    }
                });
            }
            if ((key.getValue() == LootTables.TRIAL_CHAMBERS_CORRIDOR_POT.getValue())){
                tableBuilder.modifyPools(builder -> {
                    if (!FarmersDelightCompat.IS_LOADED) {
                        builder.with(ItemEntry.builder(ModItems.CABBAGE_SEEDS)
                                .apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(1, 3), false))
                                .weight(75));
                    }
                    builder.with(ItemEntry.builder(ModItems.CHILLI_PEPPER_SEEDS)
                            .apply(SetCountLootFunction.builder(UniformLootNumberProvider.create(1, 3), false))
                            .weight(75))
